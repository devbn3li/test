...
const UpdateProject = () => {
  ...
  const selectedConsultiveIds =
    ...
    const fetchConsultive = async () => {
      try {
        ...
        if (result.status === 200) {
          setConsultives(
            result.data.filter(consultive => consultive.type === 2)
              .map((consultive) => ({
                value: consultive.id,
  ...
  const handleSubmit = async (e) => {
    ...
    selectedServiceIds.forEach((id,index) => data.append(`service_ids[${index}]`, id));
    selectedConsultiveIds.forEach((id,index) => data.append(`project_consultive_ids[${index}]`, id));
